...
public class ClassPathBeanDefinitionScanner extends ClassPathScanningCandidateComponentProvider {
    ...
    public int scan(String... basePackages) {
        ...
        // 如有必要，注册 annotation config processors
    ...
    protected Set<BeanDefinitionHolder> doScan(String... basePackages) {
        ...
        for (String basePackage : basePackages) {
            //找出当前包下所有候选组件（即带有@Component、@Repository、@Service等注解的类）
            Set<BeanDefinition> candidates = findCandidateComponents(basePackage);
            // 遍历每个候选组件
            for (BeanDefinition candidate : candidates) {
                // 解析作用域元数据（@Scope）, 并设置到 Bean 定义中
                ScopeMetadata scopeMetadata = this.scopeMetadataResolver.resolveScopeMetadata(candidate);
                candidate.setScope(scopeMetadata.getScopeName());
                // 生成 Bean 的名称，默认使用类名首字母小写
                String beanName = this.beanNameGenerator.generateBeanName(candidate, this.registry);
                // 对 AbstractBeanDefinition 做进一步默认配置（如是否延迟加载、自动注入模式等）
                if (candidate instanceof AbstractBeanDefinition) {
                    postProcessBeanDefinition((AbstractBeanDefinition) candidate, beanName);
                }
                // 如果是注解定义的 Bean（AnnotatedBeanDefinition），则处理通用注解（如@Lazy、@Primary、@DependsOn）
                if (candidate instanceof AnnotatedBeanDefinition) {
                    AnnotationConfigUtils.processCommonDefinitionAnnotations((AnnotatedBeanDefinition) candidate);
                }
                // 检查 Bean 名称是否可用，避免冲突
                if (checkCandidate(beanName, candidate)) {
                    // 创建 BeanDefinitionHolder，包含Bean 定义和生成的名称
                    BeanDefinitionHolder definitionHolder = new BeanDefinitionHolder(candidate, beanName);
                    // 应用作用域代理模式（如 @Scope(proxyMode=ScopedProxyMode.TARGET_CLASS)）
                    definitionHolder = AnnotationConfigUtils.applyScopedProxyMode(scopeMetadata, definitionHolder, this.registry);
                    //将解析好的 Bean 添加到结果集中
                    beanDefinitions.add(definitionHolder);
                    // 向registry 注册这个 Bean 定义
