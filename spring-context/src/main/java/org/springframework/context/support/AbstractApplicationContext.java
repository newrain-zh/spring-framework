...
public abstract class AbstractApplicationContext extends DefaultResourceLoader implements ConfigurableApplicationContext {
    ...
    static {
        ...
        // 急切地加载 ContextClosedEvent 类，以避免在 WebLogic 8.1 中应用程序关闭时出现奇怪的类加载器问题。（达斯汀·伍兹 （Dustin Woods） 报道。
    ...
    public void refresh() throws BeansException, IllegalStateException {
        synchronized (this.startupShutdownMonitor) {
            ...
            //1.准备此上下文以进行刷新。
            //  初始化上下文环境，标记刷新状态，校验必要配置。
            prepareRefresh();

            // Tell the subclass to refresh the internal bean factory.
            // 告诉子类刷新内部 bean 工厂。
            // 2.创建或刷新 BeanFactory，加载 Bean 定义。
            ConfigurableListableBeanFactory beanFactory = obtainFreshBeanFactory();

            // Prepare the bean factory for use in this context.
            // 3.配置Bean Factory 以在此上下文中使用。
            ...
            try {
                ...
                // 8.初始化事件广播器（默认使用SimpleApplicationEventMulticaster）用于发布应用事件
                //
                initApplicationEventMulticaster();

                // Initialize other special beans in specific context subclasses.
                // 9.模板方法，子类扩展点（如初始化主题源、创建Web容器如 Web应用中启动嵌入式 Servlet容器）
                onRefresh();
                ...
                // 12.完成刷新
                // 发布上下文刷新完成时间ContextRefreshedEvent事件
    ...
    protected void prepareRefresh() {
        ...
        // 验证标记为 required 的所有属性是否都是可解析的：请参阅 ConfigurablePropertyResolversetRequiredProperties
        ...
        // 允许早期 ApplicationEvents 的集合，以便在 multicaster 可用时发布...
     ...
     * Tell the subclass to refresh the internal bean factory.
     * 1. 销毁旧的 BeanFactory
     * 2. 创建新的 BeanFactory（通常为 DefaultListableBeanFactory）
     * 3. 加载 Bean 定义（XML/注解等方式）
     *
     ...
     * 1. 设置类加载器
     * 2. 注册环境相关的 Bean(Environment,SystemProperties)
     * 3. 添加 ApplicationContextAwareProcessor
     * 4. 忽略某些接口的自动注入（如 ResourceLoaderAware，由上下文处理）。
     * 5. 注册可解析的依赖（如 ApplicationContext 自身）。
     *
    ...
    protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
        ...
        // BeanFactory 接口未在普通工厂中注册为可解析类型。MessageSource 注册（并找到用于自动装配）为 Bean。
     ...
     * 在标准初始化后修改应用程序上下文的内部 Bean 工厂。所有 bean 定义都已加载，但尚未实例化任何 bean。
     ...
     * 实例化并调用所有已注册的 BeanFactoryPostProcessor bean，如果给定，则遵循显式顺序。<p>必须在单例实例化之前调用。
     */
