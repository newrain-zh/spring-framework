...
public abstract class TransactionAspectSupport implements BeanFactoryAware, InitializingBean {
    ...
    /**
     * 执行事务拦截逻辑，处理声明式事务管理
     * 此方法负责开启事务、执行业务逻辑、提交或回滚事务，并处理异常情况下的回滚策略
     *
     * @param method
     * @param targetClass
     * @param invocation
    ...
    protected Object invokeWithinTransaction(Method method, @Nullable Class<?> targetClass, final InvocationCallback invocation) throws Throwable {
        ...
        // 如果 transaction 属性为 null，则该方法为 non-transactional。
        // 获取事务属性源（如基于注解的 @Transactional）
        TransactionAttributeSource tas = getTransactionAttributeSource();
        // 解析当前方法的事务属性（TransactionAttribute）,判断是否需要事务
        final TransactionAttribute txAttr = (tas != null ? tas.getTransactionAttribute(method, targetClass) : null);
        // 确定使用哪个事务管理器（PlatformTransactionManager / ReactiveTransactionManager）
        ...
        if (txAttr == null || !(ptm instanceof CallbackPreferringPlatformTransactionManager)) {
            ...
            try {
                ...
                // 执行目标方法（即被增强的业务逻辑，目标方法）
                retVal = invocation.proceedWithInvocation();
            } catch (Throwable ex) {
                // target invocation exception
                // 目标方法抛出异常处理：根据事务规则决定是否回滚
            ...
            // Vavr Try 类型特殊处理：若失败则设置 rollbackOnly
    ...
    protected static final class TransactionInfo {
        ...
        private void bindToThread() {
            ...
            // 公开当前的 TransactionStatus，保留任何现有的 TransactionStatus 以便在此事务完成后进行恢复。
    ...
    private class ReactiveTransactionSupport {
        ...
        public Object invokeWithinTransaction(Method method, @Nullable Class<?> targetClass, InvocationCallback invocation, @Nullable TransactionAttribute txAttr, ReactiveTransactionManager rtm) {
            ...
            if (Mono.class.isAssignableFrom(method.getReturnType())) {
                return TransactionContextManager.currentContext().flatMap(context -> createTransactionIfNecessary(rtm, txAttr, joinpointIdentification).flatMap(it -> {
                    try {
                        ...
                        }, this::commitTransactionAfterReturning, (txInfo, err) -> Mono.empty(), this::commitTransactionAfterReturning).onErrorResume(ex -> completeTransactionAfterThrowing(it, ex).then(Mono.error(ex)));
                    } catch (Throwable ex) {
                        // target invocation exception
            ...
            return this.adapter.fromPublisher(TransactionContextManager.currentContext().flatMapMany(context -> createTransactionIfNecessary(rtm, txAttr, joinpointIdentification).flatMapMany(it -> {
                try {
                    // Need re-wrapping until we get hold of the exception through usingWhen.
                    ...
                    }, this::commitTransactionAfterReturning, (txInfo, ex) -> Mono.empty(), this::commitTransactionAfterReturning).onErrorResume(ex -> completeTransactionAfterThrowing(it, ex).then(Mono.error(ex)));
                } catch (Throwable ex) {
                    // target invocation exception
                    return completeTransactionAfterThrowing(it, ex).then(Mono.error(ex));
                }
