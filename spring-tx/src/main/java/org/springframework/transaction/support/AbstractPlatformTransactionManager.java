...
public abstract class AbstractPlatformTransactionManager implements PlatformTransactionManager, Serializable {
     ...
     * 始终激活事务同步，即使对于由没有现有后端事务的 PROPAGATION_SUPPORTS 生成的 “空” 事务也是如此。
     *
     ...
     * @see #doBegin
     */
    // 根据事务定义获取当前事务的状态信息（TransactionStatus）,
    // 并依据传播行为决定是否开启新事务或加入现有事务
    @Override
    public final TransactionStatus getTransaction(@Nullable TransactionDefinition definition) throws TransactionException {
        // Use defaults if no transaction definition given.
        // 如果未给出事务定义，则使用 defaults。
        TransactionDefinition def = (definition != null ? definition : TransactionDefinition.withDefaults());
        // 获取当前事务对象
        Object  transaction  = doGetTransaction();
        boolean debugEnabled = logger.isDebugEnabled();
        // 判断否存在事务 目标方法如果包含其他代理对象的事务方法第二次调用时会进入该判断
        ...
        // 未找到现有事务 -> 请检查传播行为以了解如何继续。
        // 下面的 if 逻辑是根据传播行为执行相应的逻辑
        // PROPAGATION_MANDATORY 要求必须有一个事务存在
        if (def.getPropagationBehavior() == TransactionDefinition.PROPAGATION_MANDATORY) {
            throw new IllegalTransactionStateException("No existing transaction found for transaction marked with propagation 'mandatory'");
        } else if (def.getPropagationBehavior() == TransactionDefinition.PROPAGATION_REQUIRED
                || def.getPropagationBehavior() == TransactionDefinition.PROPAGATION_REQUIRES_NEW
                || def.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NESTED) {
            // PROPAGATION_REQUIRED 如果存在事务，则继续事务，否则创建事务
    ...
    protected final void resume(@Nullable Object transaction, @Nullable SuspendedResourcesHolder resourcesHolder) throws TransactionException {
        if (resourcesHolder != null) {
            ...
            if (suspendedResources != null) {
                // 将事务资源（DataSource 数据库连接池）绑定到要恢复的事务
