...
public class DataSourceTransactionManager extends AbstractPlatformTransactionManager implements ResourceTransactionManager, InitializingBean {
    ...
    protected void doBegin(Object transaction, TransactionDefinition definition) {
        ...
        Connection                  con      = null;
        try {
            // 1. 如果当前没有绑定 ConnectionHolder 或者正在与另一个事务同步，
            // 则需要从数据源获取一个新的 Connection。
            // ConnectionHolder 是一个封装 Connection 的类，用于在事务中持有数据库连接。
            if (!txObject.hasConnectionHolder() || txObject.getConnectionHolder().isSynchronizedWithTransaction()) {
                // 从线程池中获取一个新的 Connection
                // 前面事务的资源已经绑定过事务资源obtainDataSource
                Connection newCon = obtainDataSource().getConnection();
                if (logger.isDebugEnabled()) {
                    logger.debug("Acquired Connection [" + newCon + "] for JDBC transaction");
                }
                // 设置ConnectionHolder到事务对象中并标记为新创建
            ...
            // 准备事务级别的数据库连接设置
            //  默认实现会根据 enforceReadOnly 属性决定是否执行 "SET TRANSACTION READ ONLY"
            //         可被子类重写以扩展自定义 SQL 设置
            prepareTransactionalConnection(con, definition);
            // 标记连接已处于活动事务中
            txObject.getConnectionHolder().setTransactionActive(true);
            //  设置事务超时时间
            //  如果事务定义中设置了 timeout，则将它设置到 ConnectionHolder 中
            //  在后续操作中会被用来限制 Statement 的执行时间
            ...
            // 使用 ThreadLocal 实现事务上下文传播，确保同一个事务中的多个 DAO 操作复用同一连接
