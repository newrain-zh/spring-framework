...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory implements AutowireCapableBeanFactory {
    ...
    protected Object createBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args) throws BeanCreationException {
        ...
        // 确保在这一点上实际解决了Bean类，并在无法存储在共享合并的BEAN定义中的动态解析类的情况下克隆BEAN定义。
        ...
        try {
            ...
            // 为BeanPostProcessors提供一个返回代理而不是目标BEAN实例的机会。
    ...
    protected Object doCreateBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args) throws BeanCreationException {
        ...
        // 2. 提前暴露 ObjectFactory 到三级缓存（解决循环依赖）
        // 条件: 单例模式、允许循环引用、Bean正在创建中
        // 加入到三级缓存中
     ...
     * 将InstantiationAwareBeanPostProcessor应用于指定的BEAN定义（按类和名称），
     * 调用其{@Code PostProcessBeforeNtantiation}方法。
     * <p>任何返回的对象都将用作bean，而不是实际实例化目标bean。
     * {@code null}从后处理器返回值将导致目标bean实例化。
     *
    ...
    protected BeanWrapper createBeanInstance(String beanName, RootBeanDefinition mbd, @Nullable Object[] args) {
        // Make sure bean class is actually resolved at this point.
        // 确保此时 bean 类实际上已解析。
//        System.out.println("AbstractAutowireCapableBeanFactory#createBeanInstance " + beanName);
...
//        System.out.println("AbstractAutowireCapableBeanFactory#invokeAwareMethods " + beanName);
        if (bean instanceof Aware) { // Aware 相当于一个标记 用来标记容器特定对象
     ...
     * 现在，给 bean 一个做出反应的机会，现在它的所有属性都已设置，并有机会了解它拥有的 bean 工厂（这个对象）。
     * 这意味着检查 Bean 是否实现了 InitializingBean 或定义了自定义 init 方法，如果是，则调用必要的回调。
     *
     * @param beanName the bean name in the factory (for debugging purposes)
     * @param bean     the new bean instance we may need to initialize
