...
//        System.out.println("AbstractBeanFactory#getBean " + name);
    ...
    protected <T> T doGetBean(String name, @Nullable Class<T> requiredType, @Nullable Object[] args, boolean typeCheckOnly) throws BeansException {
        // 1.解析 Bean 名称（处理别名、FactoryBean前缀 &）
        String beanName = transformedBeanName(name);
        Object bean;

        // Eagerly check singleton cache for manually registered singletons.
        // 2.检查单例缓存（一级缓存）是否存在实例。
        ...
        if (sharedInstance != null && args == null) {
            ...
            try {
                // 6. 获取合并的Bean定义（处理父子定义合并）
                RootBeanDefinition mbd = getMergedLocalBeanDefinition(beanName);
                checkMergedBeanDefinition(mbd, beanName, args);

                // 7. 处理依赖的Bean（@DependsOn）
                ...
                if (mbd.isSingleton()) {
                    ...
                    try {
                        beforePrototypeCreation(beanName);  // 创建前回调
                        prototypeInstance = createBean(beanName, mbd, args);
                    } finally {
                        afterPrototypeCreation(beanName);  // 创建后清理
                    }
                    bean = getObjectForBeanInstance(prototypeInstance, name, beanName, mbd);
                } else {
                    // 8.3 自定义作用域（如request/session）
     ...
     * 返回该工厂是否拥有一个InstantiationAwareBeanPostProcessor，它将用于创建中的Singleton Bean。
     *
     ...
     * 使用在此工厂中注册的自定义编辑器初始化给定的 BeanWrapper。
     * 被调用用于创建和填充 bean 实例的 BeanWrappers。<p>默认实现委托给 {@link registerCustomEditors}。可以在子类中覆盖。
     *
