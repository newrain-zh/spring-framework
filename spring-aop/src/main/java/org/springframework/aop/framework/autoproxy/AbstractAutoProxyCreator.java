...
public abstract class AbstractAutoProxyCreator extends ProxyProcessorSupport implements SmartInstantiationAwareBeanPostProcessor, BeanFactoryAware {
    ...
    public Object postProcessAfterInitialization(@Nullable Object bean, String beanName) {
        if (bean != null) {
            ...
            if (this.earlyProxyReferences.remove(cacheKey) != bean) {
                return wrapIfNecessary(bean, beanName, cacheKey); // 这里所有的 Bean 执行，会创建动态代理 Bean
            }
        }
        return bean;
    }
     ...
     * 如有必要，包装给定的 bean，即如果它符合代理条件。
     *
     * @param bean     the raw bean instance
     * @param beanName the name of the bean
     * @param cacheKey the cache key for metadata access bean name
    ...
    protected Object wrapIfNecessary(Object bean, String beanName, Object cacheKey) {
        ...
        //  不需要处理的直接返回
        // advisedBeans 缓存，用于标识Bean 是否需要代理
        if (Boolean.FALSE.equals(this.advisedBeans.get(cacheKey))) {
            return bean;
        }
        // 基础设施类直接跳过（如已经被代理或排除的列表）
     ...
     * <p>
     * 返回给定的Bean类是否代表永远不应该代理的基础架构类。
     * <p>默认实现将建议，顾问和Aopinfrastructurebeans视为基础架构类。
     *
     ...
     *
     * @param beanClass            the class of the bean 要创建的代理的目标类
     * @param beanName             the name of the bean  当前 Bean 的名称
    ...
    /*
     * 创建代理对象
     * 这里是先将proxyFactory配置好，然后再调用DefaultAopProxyFactory#createAopProxy()具体创建代理对象
     * proxyFactory 配置好目标源、拦截器数组、是否强制使用CGLIB、需要被代理实现的接口等信息
     * @param beanClass            目标类
     * @param beanName             Bean 名称
     * @param specificInterceptors 拦截器
     * @param targetSource         目标源
     * @return 代理对象
     */
    protected Object createProxy(Class<?> beanClass, @Nullable String beanName, @Nullable Object[] specificInterceptors, TargetSource targetSource) {
        // 如果当前 BeanFactory 是ConfigurableListableBeanFactory，则将目标类暴露给 AOP 代理
        if (this.beanFactory instanceof ConfigurableListableBeanFactory) {
            AutoProxyUtils.exposeTargetClass((ConfigurableListableBeanFactory) this.beanFactory, beanName, beanClass);
        }
        // 创建 ProxyFactory
        ProxyFactory proxyFactory = new ProxyFactory();
        proxyFactory.copyFrom(this); // 复制当前 AbstractAutoProxyCreator 的配置信息
        // 判断是 使用 CGLIB 还是 JDK 动态代理
        if (proxyFactory.isProxyTargetClass()) { // 显示设置 @EnableAspectJAutoProxy(proxyTargetClass = true) 判断注解中proxyTargetClass的属性
            // Explicit handling of JDK proxy targets (for introduction advice scenarios)
            if (Proxy.isProxyClass(beanClass)) { // 如果是代理类，则添加代理类实现的接口
        ...
        // 这里会根据配置先创建jdk或 cglib的AopProxy(接口)，类似策略模式。AopProxy的 getProxy()方法会最终创建动态代理
        return proxyFactory.getProxy(getProxyClassLoader()); //创建动态代理
    }
     ...
     * 返回是否要代理给定的Bean，其他建议（例如AOP Alliance Interceptors）和顾问要申请。
     *
